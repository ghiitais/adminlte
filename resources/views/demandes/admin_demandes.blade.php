                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ \Illuminate\Support\Facades\Auth::user()->is_admin == 1 ? url('admin/accepter/' . $demande->demande_id): url('manager/accepter/' . $demande->demande_id) }}" method="POST">
                                            {!! csrf_field() !!}
                                            <button type="submit" class="btn btn-success" @if($demande->status == "Acceptée" || $demande->status == "Rejetée") disabled='disabled' @endif >Accepter</button>
                                        </form>
                                    </td>

                                    <td>
                                        <a href="{{ url('demandes/' . $demande->demande_id) }}" class="{{$demande->status == "Acceptée" || $demande->status == "Rejetée"  ? 'btn btn-danger disabled': 'btn btn-danger'}}" role="button">Rejeter</a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
